...
public class Trainning_Data_Collecter : MonoBehaviour
{
 ...
         fs = File.OpenWrite(path);
    ...
    private void FixedUpdate()
    {
        ...
        target = UpdateTarget();
        print("button =" + target);
        string output = time + " " + VectortoString(IgnoreY(user.transform.position)) + " " + VectortoString(IgnoreY(user.transform.forward)) + " " + VectortoString(IgnoreY(device.velocity)) + " " + VectortoString(IgnoreY(device.angularVelocity)) + " " + vwallInfo + target;
    ...
    private Vector3 IgnoreY(Vector3 vec3)
    {
        vec3.y = 0;
        return vec3;
    }
